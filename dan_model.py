...
def vgg_block(inputs,filters,num_convs,training,kernel_size,maxpool,data_format):
    ...
    for i in range(num_convs):
        inputs = batch_norm(tf.layers.conv2d(inputs, filters,kernel_size, 1,
                                             padding='same',activation=tf.nn.relu, kernel_initializer=tf.glorot_uniform_initializer(),
...
class Model(object):
    ...
    def __init__(self, num_lmark, img_size, filter_sizes, num_convs,  kernel_size,  data_format=None):
        ...
        self.__pixels__ = tf.constant([(x, y) for y in range(self.img_size) for x in range(self.img_size)],    dtype=tf.float32,shape=[1,self.img_size,self.img_size,2])
    ...
    def __call__(self,
        ...
        with tf.variable_scope('s2'):
            ...
            for i, num_filter in enumerate(self.filter_sizes):
                inputs = vgg_block(inputs=inputs,filters=num_filter,num_convs=self.num_convs,
                                  training=s2_training,kernel_size=self.kernel_size,maxpool=True, data_format=self.data_format)
